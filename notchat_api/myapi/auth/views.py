...
from myapi.auth.notion import notion_blueprint

blueprint = Blueprint("auth", __name__, url_prefix="/auth")

@blueprint.route("/notion/login", methods=["GET"])
def notion_register():
    if not notion_blueprint.session.authorized:
        return redirect(url_for("notion.login"))
    session = notion_blueprint.session
    user = session.get("https://api.notion.com/v1/users/me").json()
    print(user)
    return jsonify(user)

# notion callback
@blueprint.route("/notion/callback", methods=["GET"])
def notion_callback():
    if not notion_blueprint.session.authorized:
        return redirect(url_for("notion.login"))
    session = notion_blueprint.session
